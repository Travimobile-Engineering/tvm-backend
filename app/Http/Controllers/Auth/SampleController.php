...
class SampleController extends Controller
{
    ...
    public function signup(RegisterRequest $request, EmailService $emailService)
    {
        ...
        do {
            $verification_code = str_pad(rand(0, 99999), 5, 0, STR_PAD_RIGHT);
        ...
        if (isset($request->verification_code) && ! empty($request->verification_code)) {
            ...
            if ($user) {
                ...
                return response()->json(['error' => 'Ooops! An error occurred. Please try again'], 400);
            }
        }
    ...
    public function verify_account(Request $request)
    {
        ...
        if ($user) {
            if ($user->verification_code_expires_at > Carbon::now()) {
                ...
                return ['status' => false, 'error' => 'Verification code has expired'];
            }
        } else {
