...
Broadcast::channel('trip.created.{userId}', fn (User $user, int $userId): bool => $user->id === $userId
);
...
Broadcast::channel('trip.start.{tripId}', fn (User $user, int $tripId): bool => $user->id === Trip::findOrFail($tripId)->user_id
);
...
Broadcast::channel('wallet.funded.{userId}', fn (User $user, int $userId): bool => $user->id === $userId
);

// Trip booking notification
Broadcast::channel('trip.booking.{userId}', fn (User $user, int $userId): bool => $user->id === $userId
);

// Trip cancelled notification (Driver)
Broadcast::channel('trip.cancelled.{tripId}', fn (User $user, int $tripId): bool => $user->id === Trip::findOrFail($tripId)->user_id
);
